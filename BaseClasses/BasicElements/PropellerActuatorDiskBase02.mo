...
partial model PropellerActuatorDiskBase02
  ...
  //--- others ---
  parameter Modelica.Units.NonSI.AngularVelocity_rpm Nmech_init=2000.0 "" annotation(
  ...
  Modelica.Units.SI.Length diam(start=2.0) "propeller diameter" annotation(
    Dialog(tab="Variables", group="start attribute" ,enable=false, showStartAttribute=true)
  );
  ...
  Real Nmech_rps(start=Nmech_init/60.0) "rev/s" annotation(
    Dialog(tab = "Variables", group = "start attribute", enable = false, showStartAttribute = true));
  Modelica.Units.SI.Force Fn "thrust generated by propeller";
  Modelica.Units.SI.Velocity Vinf "freestream speeed";
  //-----
  Modelica.Units.SI.Velocity Vinf_FnSaturation if switchFnSaturation == PropulsionSystem.Types.switches.switchPropellerThrustSaturation.byVinf "freestream speeed for thrust saturation";
  Modelica.Units.SI.Force Fn_FnSaturation if switchFnSaturation == PropulsionSystem.Types.switches.switchPropellerThrustSaturation.byFn "saturation thrust";
...
protected
  ...
  parameter Real effPropDes(fixed = false) "propeller efficiency" annotation(
    HideResult = false);
  parameter Modelica.Units.SI.Length diamDes(fixed = false) "propeller diameter" annotation(
    HideResult = false);
  /*
  parameter Modelica.Units.NonSI.AngularVelocity_rpm NmechDes "mechanical rotation speed, rpm";
