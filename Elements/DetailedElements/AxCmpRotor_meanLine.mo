...
model AxCmpRotor_meanLine
  ...
  //##### none #####
  //********** Parameters **********
  parameter Modelica.Units.SI.Angle xi_def(displayUnit = "deg") = 30.0 * Modelica.Constants.pi / 180 "angle of blade chord line" annotation(
    Dialog(group = "Geometry"));
  parameter Modelica.Units.SI.Length rTip_1_def = 2.0 "tip radius of blade, LE" annotation(
    Dialog(group = "Geometry"));
  parameter Modelica.Units.SI.Length rHub_1_def = 0.5 "tip radius of blade, LE" annotation(
    Dialog(group = "Geometry"));
  parameter Modelica.Units.SI.Length rTip_2_def = 2.0 "tip radius of blade, TE" annotation(
    Dialog(group = "Geometry"));
  parameter Modelica.Units.SI.Length rHub_2_def = 0.5 "tip radius of blade, TE" annotation(
    Dialog(group = "Geometry"));
  parameter Modelica.Units.SI.Length lAxial_def = 0.2 "axial length of blade" annotation(
    Dialog(group = "Geometry"));
  parameter Modelica.Units.SI.Area Sblade_def = (rTip_1_def - rHub_1_def + rTip_2_def - rHub_2_def) * lAxial_def * 1 / 2 "surface area of single blade" annotation(
    Dialog(group = "Geometry"));
  parameter Real numBlade_def = 30 "number of blades" annotation(
    Dialog(group = "Geometry"));
  ...
  Medium.BaseProperties fluidStat_1(p.start = pStat_1_init, T.start = TStat_1_init, state.p.start = pStat_1_init, state.T.start = TStat_1_init, h.start = hStat_1_init) "static flow station, 1";
  Medium.BaseProperties fluidStat_2(p.start = pStat_2_init, T.start = TStat_2_init, state.p.start = pStat_2_init, state.T.start = TStat_2_init, h.start = hStat_2_init) "static flow station, 2";
  //********** Interfaces **********
